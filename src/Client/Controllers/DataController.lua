--[[
    DataController.lua
    Author: herfavesenpai (Aaron)
    17 June 2021
    Stores and manages PlayerData Replica
]]
...
function DataController:GetActiveAspect(isData)
    return self:GetPromisedData():andThen(function(data)
        local activeToSlot = data.EquippedAspects[data.ActiveAspect]
        if activeToSlot >= 1 and activeToSlot <= 4 then
            local slotToInv = data.UnlockedAspects[activeToSlot]
            if slotToInv ~= nil then
                if isData == "Info" then
                    return AspectData[slotToInv.ID]
                elseif isData == "Data" then
                    return slotToInv
                end
            end
        end
